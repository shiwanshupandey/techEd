...
const Courses = (props) => (
  <tr key={props.product._id}>
    ...
    {/* <td>{props.user ? props.user.fullname : 'N/A'}</td> */}
...
class ListPurchases extends Component {
  ...
  calculateTotalMoney() {
    const totalMoney = this.state.courses.reduce(
      (total, currentCourses) => total + this.calculateSubtotal(currentCourses.products),
      0
    );
    return totalMoney;
  }
...
<h4 className="user-heading">
  Email: {currentCourses.user ? currentCourses.user.email : 'N/A'}
             ...
             <table className="table">
                ...
                <tbody>
                  {currentCourses.products.map((product) => (
                    <Courses
                      product={product}
                      user={currentCourses.user}
                      key={product._id}
             ...
             <div className="subtotal">
               <p>
                 Subtotal for {currentCourses.user ? currentCourses.user.fullname : 'N/A'}: ₹{this.calculateSubtotal(currentCourses.products)}
               </p>
             </div>
           </div>
         ))}
         <div className="total-money">
            <p>Total: ₹{this.calculateTotalMoney()}</p>
          </div>
       </div>
     </Wrapper>
